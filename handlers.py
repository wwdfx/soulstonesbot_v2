...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message:
        ...
        if len(message_text) >= 500 and update.message.chat_id == target_group_id:
            ...
            cur.execute('INSERT INTO users (user_id, symbols_count) VALUES (%s, %s) ON CONFLICT (user_id) DO UPDATE SET symbols_count = users.symbols_count + %s', (user_id, len(message_text), len(message_text)))
            conn.commit()
...
async def mission_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    end_time = start_time + timedelta(hours=mission['duration'].total_seconds() / 3600)
